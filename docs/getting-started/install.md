...
???+ note "BIOS本地化"
    在较旧的机器中，只有联想和华硕的部分主板搭载了中文BIOS。在较新的机器中，大部分已经支持简体中文。

<!-- 绝对不只有联想和华硕...我记得惠普（2009）好像也有，而且这个提示真的有用吗，有也没介绍怎么改语言，要不删掉？ -->
...
在重启系统后，会出现一个菜单，这就是Grub。大多数Linux发行版均默认采用Grub进行引导。

<!-- 大部分发行版都有grub的背景，只能说很不“终端” -->

![Grub](../img/debian_13_installer_grub_menu.png)

您可以使用方向键选择引导选项。我们建议您选择带有`Verify`（验证文件完整性），`LiveCD`（启动桌面环境）或 `Graphical`（图形化）字样的选项，以保证简单易用以及文件的完整性。如果没有，我们建议您选择第一个选项。

???+ note "一般情况黑屏卡死的处理方法"
    大多数这种情况是无法加载显卡驱动所致，此时需要在一个启动项上按下e键，找到 `linux ……` 的行，在末尾加上 `nomodeset` 参数。
    若你是NVIDIA用户，也可以选择加入 `nouveau.modeset=0` 参数。

## 安装系统

由于各发行版的配置方式不同，我们建议您参照网络上的一些教学视频或本站“详细安装过程”栏目下的教程进行安装配置。
...
或者，如果您想要双系统，请**一定不要选择全盘安装或是类似的选项！**，您应该选择带有“**使用最大的未分配*（空闲）*空间**”字眼的选项。在安装时，请选择您上一章所腾出的磁盘空余。我们建议您将该空间由系统自动分区进行管理。
...
在选择EFI[引导](./before-installing.md#_4) (/boot/efi) 分区，或者是`/boot/efi`挂载点时，请选择设备上的ESP分区（通常位于分区图示的最左侧，是一个FAT32文件系统的不大的分区）。**如果您想要保留Windows，请一定不要勾选“格式化`/boot/efi`”。**
...
若安装失败，请检查分区大小是否不足，自己的CPU是否支持（红帽系发行版正在淘汰x86-64-v1/v2指令集，导致一些老电脑无法运行这些发行版）。
